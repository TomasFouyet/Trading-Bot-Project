...
class Settings(BaseSettings):
    ...
    leverage: int = Field(default=15, ge=1, le=125)  # 1 = no leverage

    # ── Risk manager ────────────────────────────────────────
    risk_max_daily_drawdown_pct: Decimal = Decimal("20.0")
    risk_max_position_pct: Decimal = Decimal("150")  # notional cap; risk-based sizing controls real exposure
    risk_max_trade_risk_pct: Decimal = Decimal("5.0")   # % of equity risked per trade (SL-based)
