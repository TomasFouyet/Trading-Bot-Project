...
Cali technique filters (all disabled by default — enable individually):
    use_ema_cross_filter bool,  default False  — Require EMA_fast > EMA_slow for LONG (< for SHORT)
    ema_fast_filter      int,   default 7      — Fast EMA period for cross filter
    ema_slow_filter      int,   default 25     — Slow EMA period for cross filter
    vol_confirm_enabled  bool,  default False  — Require volume ≥ 80% of vol_ma for entries
    vol_confirm_period   int,   default 20     — Rolling window for volume moving average
    sr_zone_enabled      bool,  default False  — Require price near HTF S/R zone (Pupupu zones)
    sr_zone_pct          float, default 0.005  — Proximity threshold (0.5% of price)
    tp1_close_pct        float, default 0.70   — Fraction closed at TP1 (Cali recommends 0.75)
"""
...
# Indicator parameters
        self._rsi_period = int(self.params.get("rsi_period", 7))
        ...
        self._rr_ratio = float(self.params.get("rr_ratio", 1.6))
        ...
        # EMA cross filter: EMA_fast vs EMA_slow on LTF (default EMA7 / EMA25)
        self._use_ema_cross_filter = bool(self.params.get("use_ema_cross_filter", False))
        self._ema_fast_filter = int(self.params.get("ema_fast_filter", 7))
        self._ema_slow_filter = int(self.params.get("ema_slow_filter", 25))

        # Volume confirmation: current volume vs rolling average
        self._vol_confirm_enabled = bool(self.params.get("vol_confirm_enabled", False))
        self._vol_confirm_period = int(self.params.get("vol_confirm_period", 20))

        # HTF S/R zone proximity filter (Pupupu zones)
        self._sr_zone_enabled = bool(self.params.get("sr_zone_enabled", False))
        self._sr_zone_pct = float(self.params.get("sr_zone_pct", 0.005))  # 0.5% proximity
        self._htf_supports: list[float] = []
        self._htf_resistances: list[float] = []

        # TP1 partial-close percentage (Cali: 75% at first target)
        self._tp1_close_pct = float(self.params.get("tp1_close_pct", 0.70))
        ...
        self._entry_window: int = int(self.params.get("entry_window", 15))  # bars to wait
    ...
    def set_htf_bars(self, bars: pd.DataFrame) -> None:
        ...
        if self._sr_zone_enabled:
            self._update_sr_zones(bars)
    ...
    def _compute_indicators(self, bars: pd.DataFrame) -> pd.DataFrame:
        ...
        # Cali: EMA cross filter indicators (EMA7 / EMA25 or custom)
        if self._use_ema_cross_filter:
            df["ema_fast_f"] = closes.ewm(span=self._ema_fast_filter, adjust=False).mean()
            df["ema_slow_f"] = closes.ewm(span=self._ema_slow_filter, adjust=False).mean()

        # Cali: volume moving average for confirmation
        if self._vol_confirm_enabled:
            df["vol_ma"] = df["volume"].astype(float).rolling(self._vol_confirm_period).mean()
    ...
    # ── Cali: HTF S/R zone detection ──────────────────────────────────────────

    def _update_sr_zones(self, htf_bars: pd.DataFrame) -> None:
        """
        Extract swing highs/lows from HTF bars as support/resistance zones (Pupupu zones).
        Called automatically from set_htf_bars() when sr_zone_enabled=True.
        """
        w = 3  # pivot window (3 bars each side on HTF)
        n = len(htf_bars)
        if n < 2 * w + 1:
            return
        lookback = 50
        start = max(w, n - lookback - w)
        end = n - w
        if start >= end:
            return

        highs = htf_bars["high"].values.astype(float)
        lows  = htf_bars["low"].values.astype(float)
        ...
        i_range  = np.arange(start, end)

        center_h = highs[i_range]
        is_res   = (center_h > left_h[i_range - w].max(axis=1)) & \
                   (center_h > right_h[i_range].max(axis=1))
        self._htf_resistances = list(center_h[is_res])

        center_l = lows[i_range]
        is_sup   = (center_l < left_l[i_range - w].min(axis=1)) & \
                   (center_l < right_l[i_range].min(axis=1))
        self._htf_supports = list(center_l[is_sup])

    def _near_htf_support(self, price: float) -> bool:
        """True if price is within sr_zone_pct of any HTF support level (or filter disabled)."""
        if not self._sr_zone_enabled or not self._htf_supports:
            return True
        return any(abs(price - s) / price <= self._sr_zone_pct for s in self._htf_supports)

    def _near_htf_resistance(self, price: float) -> bool:
        """True if price is within sr_zone_pct of any HTF resistance level (or filter disabled)."""
        if not self._sr_zone_enabled or not self._htf_resistances:
            return True
        return any(abs(price - r) / price <= self._sr_zone_pct for r in self._htf_resistances)

    # ── Cali: volume confirmation ──────────────────────────────────────────────

    def _volume_confirms_long(self, df: pd.DataFrame) -> bool:
        """True if volume supports a bullish move (≥80% of vol_ma) or filter disabled."""
        if not self._vol_confirm_enabled or "vol_ma" not in df.columns:
            return True
        vol    = float(df["volume"].iloc[-1])
        vol_ma = float(df["vol_ma"].iloc[-1])
        if pd.isna(vol_ma) or vol_ma <= 0:
            return True
        return vol >= vol_ma * 0.8

    def _volume_confirms_short(self, df: pd.DataFrame) -> bool:
        """True if volume is not excessively above average (not a breakout bar) or filter disabled."""
        if not self._vol_confirm_enabled or "vol_ma" not in df.columns:
            return True
        vol    = float(df["volume"].iloc[-1])
        vol_ma = float(df["vol_ma"].iloc[-1])
    ...
    # ── Cali: EMA cross confirmation ───────────────────────────────────────────

    def _ema_cross_confirms_long(self, df: pd.DataFrame) -> bool:
        """True if EMA_fast > EMA_slow (short-term bullish bias) or filter disabled."""
        if not self._use_ema_cross_filter or "ema_fast_f" not in df.columns:
            return True
        ef = df["ema_fast_f"].iloc[-1]
        es = df["ema_slow_f"].iloc[-1]
        if pd.isna(ef) or pd.isna(es):
            return True
        return float(ef) > float(es)

    def _ema_cross_confirms_short(self, df: pd.DataFrame) -> bool:
        """True if EMA_fast < EMA_slow (short-term bearish bias) or filter disabled."""
        if not self._use_ema_cross_filter or "ema_fast_f" not in df.columns:
            return True
        ef = df["ema_fast_f"].iloc[-1]
        es = df["ema_slow_f"].iloc[-1]
        ...
        if self._state == "LONG":
            ...
            if cur_high >= self._tp1_price:
                ...
                return Signal(
                    ...
                    meta={**base_meta, "close_pct": self._tp1_close_pct, "be": be, "tp2": tp2},
                )
        ...
        if self._state == "ARMED":
            ...
            if self._armed_bars_elapsed > self._trigger_window:
                ...
                if self._armed_direction == "LONG" and self._long_trigger(df):
                    ...
                    # ── Cali: EMA cross filter ────────────────────────────
                    if not self._ema_cross_confirms_long(df):
                        self._reset_to_flat()
                        return hold(
                            f"long_blocked_ema_cross ema{self._ema_fast_filter}<ema{self._ema_slow_filter}",
                            {**base_meta, **trend_meta},
                        )
                    # ── Cali: volume confirmation ─────────────────────────
                    if not self._volume_confirms_long(df):
                        self._reset_to_flat()
                        return hold("long_blocked_by_volume", {**base_meta, **trend_meta})
                    # ── Cali: HTF S/R zone proximity ──────────────────────
                    if not self._near_htf_support(cur_close):
                        self._reset_to_flat()
                        return hold(
                            f"long_blocked_no_htf_support price={cur_close:.4f}",
                            {**base_meta, **trend_meta},
                        )
                ...
                if self._armed_direction == "SHORT" and self._allow_short and self._short_trigger(df):
                    ...
                    # ── Cali: EMA cross filter ────────────────────────────
                    if not self._ema_cross_confirms_short(df):
                        self._reset_to_flat()
                        return hold(
                            f"short_blocked_ema_cross ema{self._ema_fast_filter}>ema{self._ema_slow_filter}",
                            {**base_meta, **trend_meta},
                        )
                    # ── Cali: volume confirmation ─────────────────────────
                    if not self._volume_confirms_short(df):
                        self._reset_to_flat()
                        return hold("short_blocked_by_volume", {**base_meta, **trend_meta})
                    # ── Cali: HTF S/R zone proximity ──────────────────────
                    if not self._near_htf_resistance(cur_close):
                        self._reset_to_flat()
                        return hold(
                            f"short_blocked_no_htf_resistance price={cur_close:.4f}",
                            {**base_meta, **trend_meta},
                        )
